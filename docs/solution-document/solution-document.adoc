...
The architecture is based on a modular monolith (modulith) implemented with Spring Boot on the backend and a modern React frontend. The design prioritizes simplicity, maintainability, clear domain boundaries, and low operational overhead for an internal user base of approximately 100–150 users.
...
* The system is an internal company application with an expected user base of approximately 100–150 users.
...
* `utilities` — shared concerns such as configuration, common helpers, error handling, and cross-cutting infrastructure.
...
* Shared logic must be kept in common utility/configuration modules instead of being duplicated or leaked across domains.
...
==== User
Stores username, name, surname, email, password, role, status, department reference, manager reference, optional benefit, notes, and audit timestamps.

==== Department
Stores department identity, department name, manager association, and audit timestamps.

==== AssetCategory
Stores category identity and category metadata. The logical requirements also indicate that category-level booking rules must be supported, such as booking period, booking window, approval requirement, recurrence allowance, and automatic release policies.

==== Asset
Stores asset identity, category reference, descriptive data, code/QR association, status, location, and audit timestamps.

==== Booking
Stores booking identity, user reference, asset reference, booking status, start/end timestamps, notes, and audit timestamps.
...
A minimum three-environment delivery path is recommended:

* QA — automated verification and integration testing.
* Staging — pre-production validation and release acceptance.
